...
export default function SearchFilters({ onFilterChange, activeFilters = {} }) {
  ...
  const hasActiveFilters = Object.keys(activeFilters).some(key => 
    key !== 'search' && activeFilters[key] && activeFilters[key] !== ''
  );
  ...
  const getDisplayValue = (filterType, value) => {
    switch (filterType) {
      ...
      case 'priceRange':
        return priceRanges.find(r => r.value === value)?.label || 'Price Range';
      case 'condition':
        return conditions.find(c => c.value === value)?.label || 'Condition';
      case 'sort':
        return `Sort: ${sortOptions.find(s => s.value === value)?.label || 'Newest First'}`;
      default:
          ...
          <div className="flex items-center gap-2">
            {hasActiveFilters && (
              <span className="bg-green-600 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">
                {Object.keys(activeFilters).filter(key => key !== 'search' && activeFilters[key]).length}
              </span>
            )}
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {activeFilters.category && (
              <span className="inline-flex items-center gap-1 bg-green-600/10 text-green-600 text-sm font-normal font-sans leading-tight px-3 py-1 rounded-full">
                ...
                >
                  <svg className="w-3 h-3 text-green-600" fill="currentColor" viewBox="0 0 12 12">
            ...
            {activeFilters.priceRange && (
              <span className="inline-flex items-center gap-1 bg-green-600/10 text-green-600 text-sm font-normal font-sans leading-tight px-3 py-1 rounded-full">
                Price: {priceRanges.find(r => r.value === activeFilters.priceRange)?.label}
                <button 
            ...
            {activeFilters.condition && (
              <span className="inline-flex items-center gap-1 bg-green-600/10 text-green-600 text-sm font-normal font-sans leading-tight px-3 py-1 rounded-full">
                Condition: {conditions.find(c => c.value === activeFilters.condition)?.label}
                <button 
